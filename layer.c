...
static void kill_wallpaper_processes() {
    // Kill feh processes
    system("pkill -9 feh 2>/dev/null");
    system("pkill -9 swaybg 2>/dev/null");

    // Alternative method using pkill directly
    FILE *fp = popen("pkill -f 'feh.*--bg-scale'", "r");
    if (fp) pclose(fp);

    fp = popen("pkill -f swaybg", "r");
    if (fp) pclose(fp);
}
/* static void kill_wallpaper_processes() { */
